...
//        cacheManager.getCache("ips").put(request.getRemoteAddr(), token);
        ...
        if (redUrl != null) {
            cacheManager.getCache("ips").put(token.toString(), new String(Base64.getDecoder().decode(redUrl)));
        }
            ...
            try {
                ...
                if (paramServ.getParam(HTTP_HEADER) != null && Boolean.parseBoolean(paramServ.getParam(HTTP_HEADER))) {
                    ...
                    if (paramServ.getParam(URL_ENCODED) != null && Boolean.parseBoolean(paramServ.getParam(URL_ENCODED))) {
                        ...
                        if (cacheManager.getCache("ips").get(state) != null && cacheManager.getCache("ips").get(state).get() != null) {
                            return "redirect:" + cacheManager.getCache("ips").get(state).get() + "?login=" + access_token;
                        }
