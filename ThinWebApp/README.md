# Thin Web App Service


## Introduction

This project is comprised of a Java WebApp that streamlines and simplifies the integration of a Service Provider with ISS 2.0. Additionally, it provides pre-built UI for guiding the users through eIDAS authentication flow.
...
- https://someserver/eidasLogin/attributeList. This endpoint returns the list of eIDAS attributes required by the SP which are queried by the ISS 2.0
- https://someserver/eidasLogin/issResponse. This endpoint parses and handles the response form the ISS 2.0
- https://someserver/eidasLogin/authsuccess. This endpoint handles the success redirection from ISS 2.0
- https://someserver/eidasLogin/authfail/. This endpoint handles the error redirection from ISS 2.0

Finally, upon setting up the integration with the ISS 2.0 instance an SP identifier should be created for the TWA.
...
services:
  loginWebApp:
   ...
   environment:
     ...
     - SP_FAIL_PAGE=http://someserver/authFail
     - SP_SUCCESS_PAGE=http://someserver/eidasLogin/eIDASSuccess
     ...
     - SP_SERVER=https://someserver
     - SP_ID=sp1
     - SP_SECRET=
     - AUTH_DURATION=43800
     - UAEGEAN_LOGIN=true
     - CLIENT_ID=867kszvon99qp4
     - REDIRECT_URI=https://someserver/eIDASSuccess/linkedInResponse
     - LINKED_IN_SECRET=  
     - LINKED_IN=true
...
- Environment: list of environmental variables that will be added to the container
  ...
  - SP_SERVER: the url of the SP home page (i.e. https://someserver)
...
The TWA can also be build and deployed as a jar (Spring boot application with embeded tomcat). In such a case the configuration parameters presented here should be passed as environmental variables on the hosting machine. However, docker deployment is strongly advised.

### Integration with UAegean Smart Class
In order for the TWA to interact with the e-Diploma Supplement Service web app it:
- Upon authentication request, redirect the user to https://someserver/eidasLogin/login. This is the URL containing the Country selection form that allows the user to initiate an eIDAS authentication flow.
- Has implemented the endpoint https://someserver/eIDASSuccess, that consumes the genereated authentication response
